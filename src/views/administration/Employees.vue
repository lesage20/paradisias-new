  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="text-gray-600 mt-1">{{ employees.length }} employé{{ employees.length > 1 ? 's' : '' }} enregistré{{
          employees.length > 1 ? 's' : '' }}</p>
      </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div v-else-if="filteredEmployees.length === 0" class="p-8 text-center">
        ...
        <p class="text-gray-600">{{ filters.search ?
          'Aucun employé ne correspond à votre recherche.' :
          'Commencez par ajouter votre premier employé.' }}</p>
      </div>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div
                    ...
                    <span class="text-white font-medium text-sm">{{ getInitials(employee.name, employee.firstname)
                    }}</span>
                  </div>
                  <div class="ml-4">
        ...
        <form @submit.prevent="saveEmployee" class="p-6 space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input v-model="employeeForm.firstname" type="text" required
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Rôle *</label>
              <select v-model="employeeForm.group" required
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
...
const saveEmployee = async () => {
  ...
  try {
    if (selectedEmployee.value) {
      // Modification
      const updatedEmployee = await employeesAPI.updateEmployee(selectedEmployee.value.id, employeeForm.value)
      ...
      // Création
      const newEmployee = await employeesAPI.createEmployee(employeeForm.value)
